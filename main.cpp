...
int get_direction(const Point &p1, const Point &p2, const Point &p3){
	...
	return (p2.x-p1.x)*(p3.y-p1.y)-(p2.y-p1.y)*(p3.x-p1.x); // using cross product 
}
...
struct Compare{
	...
	bool operator()(const Point &p2, const Point &p3){
		...
		if (cross_product==0){ // consider distances to order p2 and p3 when polar angles are same
...
void convexHull(Point points[], int n){
	...
	for(int i{0}; i<n; i++){
		...
		else if (points[i].y == points[bottom_left_index].y && points[i].x<points[bottom_left_index].x){
			bottom_left_index = i;
		}
	}
	...
	std::stack<Point> stk;
	for(int i{0};i<n;i++){
		while(stk.size()>1 and get_direction(get_next_to_top(stk), stk.top(), points[i]) <=0){
			stk.pop();
		}
		stk.push(points[i]);
	}
...
int main() 
{ 
    Point points[] = {{0, 3}, {1, 1}, {2, 2}, {4, 4}, {0, 0}, {1, 2}, {4, 1}, {3, 3}, {0,2}, {4,2}}; 
